...
import com.nblanh.litemall.db.domain.*;
import com.nblanh.litemall.db.service.*;
...
import com.nblanh.litemall.core.express.ExpressService;
import com.nblanh.litemall.core.express.dao.ExpressInfo;
import com.nblanh.litemall.core.notify.NotifyService;
import com.nblanh.litemall.core.notify.NotifyType;
import com.nblanh.litemall.core.qcode.QCodeService;
import com.nblanh.litemall.core.system.SystemConfig;
import com.nblanh.litemall.core.util.DateTimeUtil;
import com.nblanh.litemall.core.util.JacksonUtil;
import com.nblanh.litemall.core.util.ResponseUtil;
import com.nblanh.litemall.db.util.CouponUserConstant;
import com.nblanh.litemall.db.util.OrderHandleOption;
import com.nblanh.litemall.db.util.OrderUtil;
...
import static com.nblanh.litemall.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if(null != user.getParentId()){
            LitemallSpreadOrder spreadOrder = new LitemallSpreadOrder();
            spreadOrder.setBuyUserId(userId);
            spreadOrder.setNickname(user.getNickname());
            spreadOrder.setOrderSn(order.getOrderSn());
            Object OrderId = orderId;
            spreadOrder.setOrderId(String.valueOf(OrderId));
            spreadOrder.setUserId(user.getParentId());
            spreadOrder.setGoodsPrice(orderTotalPrice);
            if("0.00".equals(String.valueOf(user.getProfitPertage())) || null != user.getProfitPertage()){
                spreadOrder.setMoney(orderTotalPrice.multiply(user.getProfitPertage().divide(new BigDecimal(100))).setScale(2,BigDecimal.ROUND_DOWN));
            }
            spreadOrder.setLevel(user.getLevel());
            spreadOrder.setStatus(0);
            spreadOrderService.add(spreadOrder);
        }
    ...
    public Object cancel(Integer userId, String body) {
        ...
        //设置分销订单取消状态
        LitemallSpreadOrder spreadOrder = spreadOrderService.findByOrderId(String.valueOf(orderId));
        if(null != spreadOrder){
            spreadOrder.setStatus(4);
            if (spreadOrderService.updateWithOptimisticLocker(spreadOrder) == 0) {
                throw new RuntimeException("更新数据已失效");
            }
        }
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (orderService.updateWithOptimisticLocker(order) == 0) {
            ...
            //设置分销订单已支付状态
            LitemallSpreadOrder spreadOrder = spreadOrderService.findByOrderId(String.valueOf(order.getId()));
            if(null != spreadOrder){
                spreadOrder.setStatus(1);
                if (spreadOrderService.updateWithOptimisticLocker(spreadOrder) == 0) {
                    throw new RuntimeException("更新数据已失效");
                }
            }
    ...
    public Object confirm(Integer userId, String body) {
        ...
        //确认收货后添加积分
        LitemallOrder orderone = orderService.findById(orderId);
        int money = (int)Float.parseFloat(orderone.getGoodsPrice().toString());
        userInfoService.addScore(userId,money);


        //设置分销订单以分成状态
        LitemallSpreadOrder spreadOrder = spreadOrderService.findByOrderId(String.valueOf(orderId));
        if(null != spreadOrder){
            spreadOrder.setStatus(3);
            if (spreadOrderService.updateWithOptimisticLocker(spreadOrder) == 0) {
                throw new RuntimeException("更新数据已失效");
            }
        }
        ...
        LitemallUser user = userService.findById(userId);
        user.setBalance(user.getBalance().add(spreadOrder.getMoney()));
